...
const char *ssid             = "Chew Chew"; // Enter your WiFi name
const char *password         = "goodmoodvisuachua";  // Enter WiFi password
int flag = 0;

/*Variable MQTT*/
PubSubClient client(espClient);
const char *mqtt_broker      = "13.229.146.39";
const char *topic            = "MQTT/Sensor";
...
void Sensor_ReadData(void *pvParameters){
  while(1){ 
    ...
    sr_temp = event.temperature;
    dht.humidity().getEvent(&event);
    sr_humi = event.relative_humidity;
    /*MQ2*/
    sr_mq2 = analogRead(PIN_MQ2);
    vTaskDelay(1000);
  }
  vTaskDelete(NULL);
}
...
void Publish_Data(void *pvParameters){
  while(1){
    ...
    client.publish("MQTT/Sensor", Buffer);
    vTaskDelay(2000);
  }
  vTaskDelete(NULL);
}
